...
if __name__ == "__main__":
    ...
    #load network data
    G_clustered_skills = load_data(str(DATA_OUTPUTS_PATH) + clustered_skills_graph_path)
    ...
    option = st.selectbox(label = '', options = list(set([skill[1] for skill in list(G_clustered_skills.nodes(data='cluster_subgroup0_name'))])))
    ...
    for skill in G_clustered_skills_color.nodes(data=True):
        if skill[1]['cluster_subgroup0_name'] == option:
            nodes.append(Node(id=skill[0], color=skill[1]['cluster_color'], size=50))

    G_clustered_skills_subgraph = G_clustered_skills_color.subgraph([node.to_dict()['id'] for node in nodes])
    for skill_pair in G_clustered_skills_subgraph.edges(data=True):
            edges.append(Edge(source=skill_pair[0], target=skill_pair[1], color = 'silver', weight=skill_pair[2]['weight']))
