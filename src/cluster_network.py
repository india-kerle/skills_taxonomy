...
def detect_skills_communities(G):
    ...
    dendo = community_louvain.generate_dendrogram(G,random_state=42,resolution=1)
    ...
    for skill in all_skills:
        ...
        for level in range(len(dendo)):
            cluster_id_at_level = community_louvain.partition_at_level(dendo, level)[skill]
    ...
    return G, {key for nodes in dendo_cluster_dict.values() for key in nodes}

def label_skills_communities(G, cluster_granularity, top_n):
    """labels all skills communities per cluster granularity."""
    
    unique_cluster_ids = list(set([node[1] for node in G.nodes(data=cluster_granularity)]))
    cluster_names = {}

    for cluster_id in unique_cluster_ids:
        cluster_words = [node[0] for node in G.nodes(data=cluster_granularity) if node[1] == cluster_id]
    ...
    nx.set_node_attributes(G, node_cluster_names, cluster_granularity + '_name')
...
def evaluate_skills_communities(G, cluster_granularity):
    ...
    unique_clusters = list(set(list(dict(G.nodes(data='cluster_subgroup0')).values())))

    for unique_cluster in unique_clusters:
        nodes = {node: data['cluster_group'] for node, data in G.nodes(data=True) if data.get('cluster_subgroup0') == unique_cluster}
        G_subgroup = G.subgraph(list(nodes.keys())) 
        modularity = community_louvain.modularity(nodes, G_subgroup, weight='weight')
        if modularity != 0:
            print("cluster id: ", unique_cluster, "modularity: ", round(modularity,2))
...
if __name__ == "__main__":
    ...
    G_skills_clustered, cluster_granularities = detect_skills_communities(G_skills)

    #add cluster labels per cluster granularity 
    for cluster_granularity in cluster_granularities:
        label_skills_communities(G_skills_clustered, cluster_granularity, top_n)
    ...
    save_data(G_skills_clustered, str(DATA_OUTPUTS_PATH) + clustered_skills_graph_path)