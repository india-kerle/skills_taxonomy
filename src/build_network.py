...
def build_skills_network(occupations_data, edgeweight_prune_threshold):
    ...
    for occupation in occupations:
        ...
        if "hasEssentialSkill" in job['_links'].keys(): 
            required_skills = [x['title'] for x in job['_links']['hasEssentialSkill']]
    ...
    skill_pairs = [(record[0][0], record[0][1], record[1]) for record in all_pairs]
...
def prune_skills_network(skills_network, bad_coefs):
    ...
    for node in nodes:
        if clustering_coefs[node] < bad_coefs: #based on plot
            clustering_coefs_to_get_rid_of.append({node: clustering_coefs[node]})

    clustering_coefs_to_get_rid_of = {k:v for el in clustering_coefs_to_get_rid_of for k,v in el.items()}
...
if __name__ == "__main__":
    ...
    skills_network = build_skills_network(occupations_data, edgeweight_threshold)
    pruned_skills_network = prune_skills_network(skills_network, bad_coefs)

    #save pruned graph locally
    save_data(pruned_skills_network, str(DATA_OUTPUTS_PATH) + skills_graph_name)
